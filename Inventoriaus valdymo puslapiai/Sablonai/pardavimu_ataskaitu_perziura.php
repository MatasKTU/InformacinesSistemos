...
function loadAtaskaitos($csvFile) {
    ...
    if (($handle = fopen($csvFile, 'r')) !== FALSE) {
        while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
            ...
            } elseif (!empty($data) && $data[0] === 'Pardavimo ID') {
...
<head>
    ...
    <title>Ataskaitų peržiūra</title>
...
<div class="container mt-5">
    <h1>Pardavimų ataskaitų peržiūra</h1>
    <form method="POST">
        <ul class="list-group">
    ...
    <?php if ($selectedAtaskaita && isset($ataskaitos[$selectedAtaskaita])): ?>
        <h3><?php echo htmlspecialchars($ataskaitos[$selectedAtaskaita]['info'][3]); ?></h3>
        <table class="table table-bordered">
            <thead>
                <tr>
                    <?php foreach ($ataskaitos[$selectedAtaskaita]['columns'] as $column): ?>
                        <th><?php echo htmlspecialchars($column); ?></th>
                    <?php endforeach; ?>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($ataskaitos[$selectedAtaskaita]['data'] as $row): ?>
                    <tr>
